    ...
    if request.method == "POST":
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object):
                ...
                flash(f"You purchased {p_item_object.name} for Rs.{p_item_object.price} successfully",category='success')
            else:
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(f"You sold {s_item_object.name} back to the market successfully", category='success')
            else:
...
def login_page():
    ...
    if form.validate_on_submit():
        ...
        if attempted_user and attempted_user.check_password_correction(
            ...
            flash(f'You are logged in successfully as: {attempted_user.username}', category='success')
            return redirect(url_for('market'))
        else:
